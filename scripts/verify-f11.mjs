...
console.log('Functional — relationship types and the is_minor flag');
const created = {};
for (const rel of ['self', 'partner', 'child', 'other']) {
...
}
{
  ...
  check("user A's profile survived B's attempts", still?.name === 'Test self');
}
{
  const { error } = await b
    .from('travelers')
